...
public class AddressBookController {
    ...
    public R<String> delete(AddressBook addressBook){
        log.info("要删除的地址id：{}", addressBook.getId());

        //判断当前用户当前地址是否为默认地址，如果为默认地址，则无法删除
        AddressBook book = addressBookService.getById(addressBook.getId());
        if (book.getIsDefault() == 1) {
            return R.error("当前地址为默认地址，无法删除，请修改后再删除");
        }
